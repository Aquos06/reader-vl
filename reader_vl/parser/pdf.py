...
from src.advanced_parser.parser.schemas import Document, Page
...
class PDFParser:
    def __init__(
        self, pdf_bytes=None, filename: str = "", file_path: Path = ""
    ) -> None:
                ...
                for box in boxes:
                    try:
                        ...
                        child_components.append(
                            CLASS_REGISTRY[box_class](coordinate, cut_image)
                        )
                        ...
                        cv2.imwrite(
                            "/home/hansen/workspace/parser/error.jpg", cut_image
                        )
                        logging.info(f"Saved images")
                        return

            components.append(Page(page=index, structure=child_components))

        return Document(
            filename=self.file_name, filepath=self.file_path, page=components
        )
