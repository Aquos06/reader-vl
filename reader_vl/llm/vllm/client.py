...
from reader_vl.llm.schemas import CompletionResponse, ChatCompletionResponse, ChatMessage

logging.basicConfig(level=logging.INFO)

class VllmClient(llmBase):
    def __init__(self, url: str, model:str, temperature: float, max_tokens: Optional[int])->None:
    ...
    async def _aprocess_stream_chunk(self, chunk: str, response_type):
        ...
        if match:
            ...
            try:
                ...
                raise ValueError(f"An unxepected error occured during streaming, with error :{e}")
    ...
    def _process_stream_chunk(self, chunk: str, response_type):
        ...
        if match:
            ...
            try:
                ...
                logging.error(f"Unexpected chunk format: {chunk}, with error: {e}")
                raise ValueError(f"An unxepected error occured during the streaming, with error: {e}")
    ...
    def completion(self, prompt,**kwargs) -> CompletionResponse:
        response = requests.post(url=self.completion_url, json=self.get_completion_params(prompt=prompt, **kwargs))
        try:
    ...
    def chat(self, message, **kwargs) -> ChatCompletionResponse:
        response = requests.post(url=self.chat_url, json=self.get_chat_params(message=message, **kwargs))
        try:
            response.raise_for_status()
            response = response.json()
    ...
    async def acompletion_stream(self, prompt: str, **kwargs) -> AsyncGenerator[CompletionResponse]:
        params = self.get_completion_params(prompt=prompt, **kwargs)
        params["stream"] = True
        
        try:
            async with httpx.AsyncClient as client:
                async with client.post(self.completion_url, json=params, stream = True) as response:
                    response.raise_for_status()
                    async for chunk in response.aiter_lines():
                        if chunk:
                            processed_chunk = await self._aprocess_stream_chunk(chunk, CompletionResponse)
                            yield processed_chunk
    ...
    async def achat_stream(self, message: List[ChatMessage], **kwargs) -> AsyncGenerator[ChatCompletionResponse]:
        params = self.get_chat_params(message=message, **kwargs)
        params["stream"] = True
        
        try:
            async with httpx.AsyncClient as client:
                async with client.post(self.chat_url, json=params, stream=True) as response:
                    response.raise_for_status()
                    
                    async for chunk in response.aiter_lines():
                        if chunk:
                            processed_chunk = await self._aprocess_stream_chunk(chunk, ChatCompletionResponse)
                            yield processed_chunk
    ...
    def chat_stream(self, message: List[ChatMessage], **kwargs) -> Generator[ChatCompletionResponse]:
        params = self.get_chat_params(message=message, **kwargs)
        params["stream"] = True
